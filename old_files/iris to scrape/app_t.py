...
def load_configs_for_country(country: str) -> list[dict]:
    ...
    for cfg_file in config_files:
        ...
        if os.path.exists(cfg_path):
            ...
            if cfg:
                cfg["_filename"] = cfg_file  # Store filename for pagination lookup
                configs.append(cfg)
...
def scrape_with_autoscraper_config(cfg: dict, start_d: date, end_d: date) -> list[dict]:
    ...
    try:
        ...
        if not scraper_path:
            st.error(f"No scraper file path found in config {config_filename}")
            return []
        
        scraper_full_path = os.path.join(CONFIGS_DIR, scraper_path)
        if not os.path.exists(scraper_full_path):
            st.error(f"Scraper file not found: {scraper_full_path}")
            return []
        
        # Load the trained scraper
        st.write(scraper_full_path)
        scraper = AutoScraper()
        scraper.load(scraper_full_path)
        
        # Load pagination settings from pagination.json
        ...
        st.error(f"Error scraping {cfg.get('site_name', 'unknown')}: {e}")
        return []
